    ...
    return Scaffold(
      ...
      body: Obx(() {
        if (controller.isLoading.value &&
            controller.bookingSessionModel.isEmpty) {
          ...
          Flexible(
            child: Column(
              ...
              children: [
                ...
                CkText(
                  text:
                      "${CkUtils.formatDateToShortMonth(bookingSessionModel?.startTime.toLocal() ?? DateTime.now())} (${bookingSessionModel?.startTime.toLocal().time} - ${DateFormat("h:mm a").format(bookingSessionModel?.endTime.subtract(Duration(minutes: 5)).toLocal() ?? DateTime.now())}",
                  // fontFamilyIndex: 2,
                  fontSize: 14,
                  fontWeight: FontWeight.w500,
